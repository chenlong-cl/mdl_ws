...
public:
  ...
  {  
    ...
    sync->registerCallback(std::bind(&CanPublisher::pub_can_to_car_callback, this, std::placeholders::_1, std::placeholders::_2)); 
    
    publisher_can_to_cloud = this->create_publisher<std_msgs::msg::String>("can_cloud_message", 10);

    publisher_can_to_system = this->create_publisher<custom_interfaces::msg::Stampstring>("can_system_message", 10);  
    
    can_system_init = this->create_wall_timer(
      100ms, std::bind(&CanPublisher::can_system_callback, this)); 
    can_cloud_init = this->create_wall_timer(
      5s, std::bind(&CanPublisher::can_cloud_callback, this));

  }

private:
  int s,nbytes1,nbytes2,nbytes3;
  ...
  rclcpp::TimerBase::SharedPtr can_system_init;
  rclcpp::TimerBase::SharedPtr can_cloud_init;
  rclcpp::Publisher<std_msgs::msg::String>::SharedPtr publisher_can_to_cloud;
  rclcpp::Publisher<custom_interfaces::msg::Stampstring>::SharedPtr publisher_can_to_system;

  size_t count_;
  
  void can_cloud_callback()
  {
    Json::Value param;
    Json::Value message0,message1,message2,message3,message4,message5,message6,message7;
    ...
    can0 = read(s, &frame[0], sizeof(frame[0])); 
    if (("0x%08X",(frame[0].can_id & CAN_EFF_MASK))==0x180D20E0){
      message0['0']=Json::Value(frame[0].data[0]);
      message0['1']=Json::Value(frame[0].data[1]);
      message0['2']=Json::Value(frame[0].data[2]);
      message0['3']=Json::Value(frame[0].data[3]);
      message0['4']=Json::Value(frame[0].data[4]);
      message0['5']=Json::Value(frame[0].data[5]);
      message0['6']=Json::Value(frame[0].data[6]);
      message0['7']=Json::Value(frame[0].data[7]);}
    
    can1 = read(s, &frame[1], sizeof(frame[1])); 
    if (("0x%08X",(frame[1].can_id & CAN_EFF_MASK))==0x180028D0){
      message1['0']=Json::Value(frame[1].data[0]);
      message1['1']=Json::Value(frame[1].data[1]);
      message1['2']=Json::Value(frame[1].data[2]);
      message1['3']=Json::Value(frame[1].data[3]);
      message1['4']=Json::Value(frame[1].data[4]);
      message1['5']=Json::Value(frame[1].data[5]);
      message1['6']=Json::Value(frame[1].data[6]);
      message1['7']=Json::Value(frame[1].data[7]);}
      
    can2 = read(s, &frame[2], sizeof(frame[2])); 
    if (("0x%08X",(frame[2].can_id & CAN_EFF_MASK))==0x180128D0){
      message2['0']=Json::Value(frame[2].data[0]);
      message2['1']=Json::Value(frame[2].data[1]);
      message2['2']=Json::Value(frame[2].data[2]);
      message2['3']=Json::Value(frame[2].data[3]);
      message2['4']=Json::Value(frame[2].data[4]);
      message2['5']=Json::Value(frame[2].data[5]);
      message2['6']=Json::Value(frame[2].data[6]);
      message2['7']=Json::Value(frame[2].data[7]);}
      
    param["0x180D20E0"]=message0;
    param["0x180028D0"]=message1;
    param["0x180128D0"]=message2;
    
    Json::FastWriter sw;
    string_can_msg.data=sw.write(param); 
    publisher_can_to_cloud->publish(string_can_msg);
    
  }
  void can_system_callback()
  {
    Json::Value root;
  ...
  void pub_can_to_car_callback(const custom_interfaces::msg::Stampstring::ConstSharedPtr dsm_msg, const custom_interfaces::msg::Stampstring::ConstSharedPtr adas_msg)
  {
    try{
...
public:
  ...
  : Node("ultrasound_publisher"), count_(0)
  {
    ...
    ultrasound_init = this->create_wall_timer(100ms, std::bind(&UltrasoundPublisher::ultrasound_callback, this));
    ultrasound_event_pub = this->create_subscription<custom_interfaces::msg::Stampstring>(
      "can_system_message", 10, std::bind(&UltrasoundPublisher::ultrasound_event_callback, this, std::placeholders::_1));
...
private:
  ...
  rclcpp::TimerBase::SharedPtr ultrasound_init;
  rclcpp::Subscription<custom_interfaces::msg::Stampstring>::ConstSharedPtr ultrasound_event_pub;
