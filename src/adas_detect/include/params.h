...
class Params
{
  public:
    ...
    const char *class_name [num_classes] = { "Pedestrian","Car","Dontcare",};
    const float min_x_range = -30.24;
    const float max_x_range = 30.24;
    const float min_y_range = -39.68;
    const float max_y_range = 39.68;
    const float min_z_range = -1.8;
    const float max_z_range = 5.0;
    // the size of a pillar
    const float pillar_x_size = 0.14;
    const float pillar_y_size = 0.16;
    const float pillar_z_size = 6.8;
    ...
    const float anchor_bottom_heights[num_classes] = {-0.71,-0.82,-0.67,};
