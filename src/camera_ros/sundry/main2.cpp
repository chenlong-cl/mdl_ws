...
Mat dsm_frame;
Mat adas_frame;
Size dsm_dsize = Size(640, 480);
Size adas_dsize = Size(640, 480);
...
void readRTSPVideo()
{
    VideoCapture dsmcap(dsm_rtsp_url);
    if (!dsmcap.isOpened())
    {
        throw std::runtime_error("Failed to open DSM RTSP stream");
        exit(1);
    }
    logger->info("DSM camera start successfully");
    logger->flush();

    VideoCapture adascap(adas_rtsp_url);
    if (!adascap.isOpened())
    {
        throw std::runtime_error("Failed to open ADAS RTSP stream");
        exit(1);
    }
    logger->info("ADAS camera start successfully");
    logger->flush();

    while (rclcpp::ok())
    {
        Mat dsm_temp, dsm_temp_size, adas_temp, adas_temp_size;
        dsmcap >> dsm_temp;
        // dsm_temp_size = dsm_temp(dsm_rect);
        resize(dsm_temp, dsm_temp_size, dsm_dsize, 0, 0, INTER_AREA);
        dsm_frame_mutex.lock();
        dsm_frame = dsm_temp_size.clone();
        dsm_frame_mutex.unlock();

        // adascap >> adas_temp;
        // // adas_temp_size = temp(dsm_rect);
        // resize(adas_temp, adas_temp_size, adas_dsize, 0, 0, INTER_AREA);
        // adas_frame_mutex.lock();
        // adas_frame = adas_temp_size.clone();
